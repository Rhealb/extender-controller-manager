...
func isPVCanBeRemovedFromNode(pv *v1.PersistentVolume, nodeName string) bool {
	if algorithm.IsCommonHostPathPV(pv) && pv.Annotations != nil &&
...
func isPVNeedSync(pv *v1.PersistentVolume) (bool, uint) {
	if algorithm.IsCommonHostPathPV(pv) == false || pv.Annotations == nil ||
...
func getPVNodes(pv *v1.PersistentVolume) []string {
	ret := make([]string, 0)
	if pv == nil || pv.Annotations == nil || pv.Annotations[xfs.PVVolumeHostPathMountNode] == "" {
		return ret
	}
...
func (npvc *NodePVController) deletePods(podInfos []podInfo, nodeName string, recordFunc func(podNS, podName string, err error)) {
	for _, pod := range podInfos {
		if ok, err := npvc.isPodCanBeDeleted(pod, nodeName); ok {
			...
			glog.Infof("NodePVController need delete pod %s:%s :%v\n", pod.podNS, pod.podName, err)
			if recordFunc != nil {
				recordFunc(pod.podNS, pod.podName, err)
			}
		} else {
			glog.Infof("NodePVController not need delete pod %s:%s :%v\n", pod.podNS, pod.podName, err)
			if recordFunc != nil {
				recordFunc(pod.podNS, pod.podName, err)
			}
		}
		...
		if needRetry == false {
			if err == nil && pv != nil {
				...
				if isTimeoutNeedDeletePod(pv) {
					glog.Infof("NodePVController need delete pv %s pods: %d\n", pv.Name, len(deletePodInfos))
					...
					glog.Infof("NodePVController node not need delete pv %s pods\n", pv.Name)
				}
			}
			return err
		}
		...
		go func() {
			...
			for {
				...
				case <-time.After(time.Second):
					glog.Infof("NodePVController debug sync node %s\n", nodeName)
					run := func() bool {
						npvc.nodePVMapMutex.Lock()
						defer npvc.nodePVMapMutex.Unlock()
...
func (npvc *NodePVController) debug() {
	func() {
		npvc.runMutex.Lock()
		defer npvc.runMutex.Unlock()
		glog.Infof("NodePVController debug +++++++++++++++++++++++++++++++\n")
		for node, _ := range npvc.nodeSyncMap {
			glog.Infof("NodePVController debug ++++++ node[%s] is syncing\n", node)
		}
		glog.Infof("NodePVController debug +++++++++++++++++++++++++++++++\n")
	}()
	...
	func() {
		...
		for node, nodeMap := range npvc.nodePVTimeOutMap {
			...
			glog.Infof("NodePVController debug ---- node[%s]: %s\n", node, tmpstr)
		}
		glog.Infof("NodePVController debug -------------------------------\n")
	}()
}
...
func getPVMountPodsInfo(info xfshostpath.HostPathPVMountInfo) (podInfos []podInfo) {
